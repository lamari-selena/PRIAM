  ...
  post(processing: Processing) {
    if (this.referenceId)
      this.getConsentsService.postConsent(
        ...
        // The toggle strictly flips the previous state (ConsentServiceImpl
        // .create) - update locally instead of waiting for a manual refresh,
        // since nothing else re-fetches this page's state after an action.
        this.consents[processing.processingId] =
          !this.consents[processing.processingId];
      })
