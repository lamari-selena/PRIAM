...
export class AccessRequestComponent implements OnInit {
  ...
  // Extracted so the data subject can re-fetch on demand: this page only
  // ever fetched once, in ngOnInit, so a rectification/erasure approved by
  // the app owner after the page loaded (the common case) never showed its
  // new value without a full browser reload - same root cause as the
  // "My Requests" page fix (playbook §8.8.c).
  refresh() {
