...
export class SuppressionComponent {
  ...
  postCompletedSuppressionRequest() {
    // Same fix as RectificationComponent: guard against double-submit (the
    // backend 500s on a second answer to the same request) and against the
    // submit button navigating away before the async POST completes (it used
    // to combine (click) with routerLink="/dashboard" on the same element).
