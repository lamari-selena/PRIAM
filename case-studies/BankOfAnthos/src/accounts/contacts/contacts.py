...
# data_id values annotated for the "Contact" data type in
# Databases/db_insertion_script.sql - reported at every contact creation
# (playbook §4bis - "the most frequently forgotten point").
CONTACT_DATA_IDS = [11, 12, 13, 14]
CONTACT_PROCESSING = 'Contact Management'
...
def create_app():
    ...
    def add_contact(username):
        ...
        try:
            ...
            # Add contact_data to database - gated on consent (playbook §4):
            # saving a contact is the only genuinely optional side effect in
            # this app (a payment/deposit can be submitted without labeling
            # one), so it alone is wrapped in get_consent(); the mandatory
            # transaction itself, submitted separately by the caller, is
            # never affected by this check.
            app.logger.debug("Adding new contact to the database.")
            if priam.get_consent(username, CONTACT_PROCESSING, app.logger):
                contacts_db.add_contact(contact_data)
                # Backgrounded (unlike get_consent() above, whose boolean
                # result gates the write and must stay synchronous): pure
                # bookkeeping, no return value used - see the matching
                # comment in userservice.py for why this must not run
                # inline (a slow PRIAM round-trip here could push this
                # handler past the caller's own timeout).
                threading.Thread(target=priam.report_processed_data, args=(
                    username, CONTACT_DATA_IDS, app.logger
                ), daemon=True).start()
