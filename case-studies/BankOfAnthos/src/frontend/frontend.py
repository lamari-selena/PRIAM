...
# Must match the processing_name used by contacts.py's get_consent() call
# and Databases/db_insertion_script.sql (playbook §4bis).
...
def create_app():
    ...
    def home():
        ...
        # Forced consent decision (playbook §4bis): redirect once to PRIAM's
        # consent page if this subject has never answered the OPTIONAL
        # Contact Management processing yet. Only fires once by
        # construction - has_pending_consent_decision() becomes False as
        # soon as a decision (granted or refused) exists.
        if priam.has_pending_consent_decision(username, CONTACT_PROCESSING, app.logger):
            return redirect(f"{app.config['PRIAM_FRONTEND_URL']}/consent")
    ...
    app.config['PRIAM_FRONTEND_URL'] = os.environ.get('PRIAM_FRONTEND_URL', 'http://localhost:4200')
