...
microservices + 2 Angular frontends) into **TeaStore**, whose code lives in
`case-studies/TeaStore/services`. PRIAM itself lives at the root of this repository/folder.
...
3. Open `TeaStore`'s real code (ORM models, existing routes) — never invent
...
1. `Databases/db_insertion_script.sql` — annotation of `TeaStore`'s real
   ...
   (§8.1.b).
2. The 4 Provider bridge endpoints on the `TeaStore` side (§2) —
...
5. **Bidirectional, persistent navigation between `TeaStore` and PRIAM** —
   not just the forced sign-up redirect (point 4): a visible button/link in
   `TeaStore` (e.g. in account settings) letting an already-registered user
   ...
   of `TeaStore`, not just its default root URL.
...
8. Docker wiring (`common_network`, `PRIAM_CDP_URL`/`PRIAM_ACTOR_URL`,
   `CUSTOM_PROVIDER_URL` pointing directly at `TeaStore`) — §5. **If another
...
9. If a Keycloak IdP is wired up (§6) **and** `TeaStore` has its own local
...
- **PRIAM stays generic: 0 lines of code changed in the PRIAM
  ...
  generic PRIAM bug (not specific to `TeaStore`) through a real test — only
...
- **Minimize the number of lines added on the `TeaStore` side** — reuse the
...
1. `case-studies/TeaStore/priam-integration/INTEGRATION-REPORT.md` — the
...
2. `case-studies/TeaStore/priam-integration/ETAPES-FAITES.md` — the raw
   ...
   `/data`, `/provider` prefixes; the Provider bridge port on the `TeaStore`
