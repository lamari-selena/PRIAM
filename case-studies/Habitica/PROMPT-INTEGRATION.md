...
microservices + 2 Angular frontends) into **Habitica**, whose code lives in
`case-studies/Habitica/website`. PRIAM itself lives at the root of this repository/folder.
...
3. Open `Habitica`'s real code (ORM models, existing routes) — never invent
...
4. If you see `Docs/SESSION-HANDOFF-*.md` or any file in `Docs/` documenting a
   ...
   find it there).
5. Before touching the code: launch `Habitica` on its own (without PRIAM) and
...
1. `Databases/db_insertion_script.sql` — annotation of `Habitica`'s real
   ...
   (§8.1.b).
2. The 4 Provider bridge endpoints on the `Habitica` side (§2) —
...
5. **Bidirectional, persistent navigation between `Habitica` and PRIAM** —
   not just the forced sign-up redirect (point 4): a visible button/link in
   `Habitica` (e.g. in account settings) letting an already-registered user
   ...
   of `Habitica`, not just its default root URL.
...
8. Docker wiring (`common_network`, `PRIAM_CDP_URL`/`PRIAM_ACTOR_URL`,
   `CUSTOM_PROVIDER_URL` pointing directly at `Habitica`) — §5. **If another
...
9. If a Keycloak IdP is wired up (§6) **and** `Habitica` has its own local
...
- **PRIAM stays generic: 0 lines of code changed in the PRIAM
  ...
  generic PRIAM bug (not specific to `Habitica`) through a real test — only
...
- **Minimize the number of lines added on the `Habitica` side** — reuse the
...
1. `case-studies/Habitica/priam-integration/INTEGRATION-REPORT.md` — the
...
2. `case-studies/Habitica/priam-integration/ETAPES-FAITES.md` — the raw
   ...
   `/data`, `/provider` prefixes; the Provider bridge port on the `Habitica`
