...
microservices + 2 Angular frontends) into **FastAPIHealthcare**, whose code lives in
`case-studies/FastAPIHealthcare/app`. PRIAM itself lives at the root of this repository/folder.
...
3. Open `FastAPIHealthcare`'s real code (ORM models, existing routes) — never invent
...
4. If you see `Docs/SESSION-HANDOFF-*.md` or any file in `Docs/` documenting a
   ...
   find it there).
5. Before touching the code: launch `FastAPIHealthcare` on its own (without PRIAM) and
...
1. `Databases/db_insertion_script.sql` — annotation of `FastAPIHealthcare`'s real
   ...
   (§8.1.b).
2. The 4 Provider bridge endpoints on the `FastAPIHealthcare` side (§2) —
...
5. **Bidirectional, persistent navigation between `FastAPIHealthcare` and PRIAM** —
   not just the forced sign-up redirect (point 4): a visible button/link in
   `FastAPIHealthcare` (e.g. in account settings) letting an already-registered user
   ...
   of `FastAPIHealthcare`, not just its default root URL.
...
8. Docker wiring (`common_network`, `PRIAM_CDP_URL`/`PRIAM_ACTOR_URL`,
   `CUSTOM_PROVIDER_URL` pointing directly at `FastAPIHealthcare`) — §5. **If another
...
9. If a Keycloak IdP is wired up (§6) **and** `FastAPIHealthcare` has its own local
...
- **PRIAM stays generic: 0 lines of code changed in the PRIAM
  ...
  generic PRIAM bug (not specific to `FastAPIHealthcare`) through a real test — only
...
- **Minimize the number of lines added on the `FastAPIHealthcare` side** — reuse the
...
1. `case-studies/FastAPIHealthcare/priam-integration/INTEGRATION-REPORT.md` — the
...
2. `case-studies/FastAPIHealthcare/priam-integration/ETAPES-FAITES.md` — the raw
   ...
   `/data`, `/provider` prefixes; the Provider bridge port on the `FastAPIHealthcare`
