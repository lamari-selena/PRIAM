"""
PRIAM Provider endpoints for FastAPI-Healthcare.
...
def _get_record(
    db: Session, model_name: str, id_ref: str, primary_keys: dict
) -> Any:
...
def data_access_right(
    ...
    if dataTypeName == "MedicalRecord":
        ...
        return [
            {"recordId": r.id, **{attr: getattr(r, attr, None) for attr in attrs}}
            for r in records
        ]
    ...
    if dataTypeName == "Appointment":
        ...
        return [
            {"appointmentId": a.id, **{attr: getattr(a, attr, None) for attr in attrs}}
            for a in appts
        ]
...
def rectification(body: RectificationRequest, db: Session = Depends(get_db)):
    ...
    db.commit()
    logger.info(
        "Rectification: patient=%s model=%s field=%s",
        body.idRef, body.dataTypeName, body.dataName,
    )
    return {"status": "ok"}
...
def erasure(body: ErasureRequest, db: Session = Depends(get_db)):
    ...
    db.commit()
    logger.info(
        "Erasure: patient=%s model=%s field=%s",
        body.idRef, body.dataTypeName, body.dataName,
    )
    return {"status": "ok"}