...
> not name any specific target application, only patterns to reproduce.
>
...
> right thematic group in §8 rather than documenting it elsewhere.
...
Minimal pattern (Python shown as an example; the same shape applies in any
language/framework):
...
be asked for an explicit consent decision right at sign-up rather than never.
...
- **`report_processed_data`** is **the most frequently forgotten point of the
  ...
  work is (§8.1.b). Encountered under real conditions, entirely absent (none
  of the 3 functions in this section wired up for data bookkeeping), symptom
  "only 1 field out of 7 annotated shows up" for a normally registered
  subject. **Call it at every
...
- **Client side**: at the point where the application already handles the
  ...
  (`{PRIAM_FRONTEND_URL}/consent`) if the flag is true. **Append
  `?login_hint=<value>` to that URL** (the same value used as the Keycloak
  `username`, e.g. the email) if a Keycloak IdP is wired up (§6) — see
  "Automatic Keycloak identity provisioning" below for why this one query
  parameter matters far more than it looks. The redirect happens
...
- **⚠️ Mandatory ordering if the target application also reports "processed"
  ...
  and reproduced under real conditions (two
...
- **Users who already existed before the sign-up hook was added**:
  ...
  existing users table and calls `register_data_subject` for each one
  (reuse the application's already-configured ORM directly, not a separate
...
conditions: creating a target-application account
...
- **⚠️ `firstName`/`lastName` are mandatory despite appearances.** An account
  ...
  real conditions (Keycloak 26): the account appeared
  ...
  `username` for both fields (for a target application with no separate
  first/last name at sign-up) — any target application without distinct
  first/last name fields can do the same. **Generalization confirmed**: the
...
- **⚠️ Keycloak `username` has a 3-character minimum — do not reuse the
  ...
  conditions with a real account named `"w"`, a single character)
...
- **⚠️ The very first redirect, right after sign-up, is where this
  synchronization matters most — and where it is easiest to get wrong.**
  `has_pending_consent_decision()` (§4bis above) sends a freshly registered
  user straight to `{PRIAM_FRONTEND_URL}/consent`, which itself immediately
  redirects to the IdP's login form (Keycloak or otherwise) if no browser
  session exists yet — which, for a brand-new user, it never does. At this
  exact moment the user has just chosen credentials **on the target
  application**, has seen no mention of Keycloak/PRIAM anywhere, and is now
  looking at a blank login form belonging to a service they have never heard
  of. "The user still sees one login screen, but with the credentials they
  just chose" is **only true if two separate conditions both hold** — do not
  assume either:
  1. **`provision_keycloak_user()` has actually finished** before the
     redirect happens. It is fire-and-forget, exactly like
     `register_data_subject()` (§4bis, §8.6) — a slow or failed provisioning
     call racing the forced redirect leaves the user staring at a login form
     for an account that does not exist yet, indistinguishable from a wrong
     password.
  2. **The user correctly guesses what to type**, with zero prompting, on a
     form belonging to a product they were never told about. Even with
     credentials that technically work, "try the same email/password you
     just used two seconds ago, on this unrelated-looking screen" is not a
     safe assumption — a user who instead tries their target-application
     *username* (not email) on a Keycloak login form that expects an email,
     because nothing told them otherwise, sees a login failure that looks
     exactly like a real bug.
  - **Two mitigations, both required, not either/or:**
    1. **Make the credentials genuinely predictable, not just synchronized.**
       Reusing the target application's own **email** as the Keycloak
       `username` (already recommended above, §8.8) is the right default
       specifically because the user can guess it without being told —
       resist synthesizing an opaque identifier here even if it seems
       cleaner internally.
    2. **Pre-fill the identity server's login form instead of relying on the
       user's memory — already generic, already implemented, PRIAM-Frontend
       side.** `PRIAM-Frontend`'s `app.module.ts` (`oidcInitializer`) already
       reads an optional `?login_hint=...` query parameter off its own URL
       and forwards it to `initLoginFlow()` via `oauthService.
       customQueryParams` before redirecting to the issuer — every standard
       OIDC login form (Keycloak included) uses `login_hint` to pre-populate
       its username field. **Nothing to build on PRIAM's side**: the only
       thing missing is the target application actually appending it when
       it redirects a freshly registered user, i.e. send them to
       `{PRIAM_FRONTEND_URL}/consent?login_hint=<value>` (the same value
       used as the Keycloak `username` by `provision_keycloak_user()`, e.g.
       the user's email) instead of the bare `/consent` path. This one query
       parameter is the entire integration cost of removing the guesswork
       — do not skip it, and do not re-derive an equivalent mechanism from
       scratch. Note: `PRIAM-Frontend-Provider` (the data-controller
       frontend, §5bis of Docs/PRIAM-AUTH-OIDC.md) does **not** forward
       `login_hint` — expected, since it has no equivalent
       "just-registered-elsewhere" flow to support.
  - A truly silent SSO (zero visible login screen at all) would require
    establishing a browser Keycloak session right after sign-up — a separate,
    more fragile effort, not covered here. The two mitigations above are the
    realistic minimum bar; treat a raw, unprompted login form as a UX defect
    to fix, not an acceptable one-time inconvenience.
...
- **Target application → PRIAM**: a "Manage on PRIAM" button in the target
  application's own settings/account page, shown
  only if `PRIAM_FRONTEND_URL` is configured on the application side, pointing
  to `PRIAM-Frontend` (`frontuser`, e.g. `http://localhost:4200`).
- **PRIAM → target application**: a "Back to the app" link in `PRIAM-Frontend`'s
  ...
  switch** to the real, browser-facing origin of that target application.
...
error. Encountered under real conditions: a
rectification approved with `answer:true` and a `200 {"answer":"FULL"}`
response, without the field actually changing in the target application's
database — the
...
| 8.8 | Keycloak provisioning (to surface on the target-application side) | The Keycloak `username` synced by `provision_keycloak_user()` is never shown anywhere in the target application's own UI, and the very first forced redirect after sign-up shows a blank IdP login form with no prompt (§4bis, "the very first redirect") | User has no way to know what to type on Keycloak's login screen for "Manage on PRIAM" **or** on the very first post-sign-up redirect |
...
generic PRIAM bug, found and fixed during a real integration.**
...
processing (e.g. a KYC/legal-obligation
processing) is completely invisible on `/consent` — not disabled,
...
**absent from every existing case study integrated before this pitfall was
found**: no one had clicked on this specific Provider dashboard
...
presence of `primaryKeys`. To be added for any case study that does not yet
have this endpoint, even ones already considered "finished".

### 8.6 PRIAM-Actor-service — race condition at registration (new, fixed)

Encountered under real conditions (not in
...
Encountered under real conditions (a real
user, not an automated test): a target application's registration wizard
showing a
security token exactly once, right after account creation returns, with an
...
**Two distinct occurrences of the same root problem — fix both, not just
one.** The scenario below (a user deliberately clicking "Manage on PRIAM"
later) is the easier one to notice and reproduce; §4bis's "the very first
redirect" block covers the more damaging occurrence — the **forced**
redirect immediately after sign-up, before the user has seen any
target-application UI that could have surfaced the username. Fixing only
the settings-page symptom below still leaves every brand-new user stuck on
their very first action after registering.

Encountered under real conditions, reported by a real user
...
client already has (e.g. `{userId}@<app-domain>`). Generalizes to any
...
- **Docker resource limits**: running the whole PRIAM stack ON TOP OF the
  ...
  service. **Concrete data point**: on a 16GB-RAM
  Windows host with Docker Desktop's WSL2 VM capped at its default ~7.5GB,
  running PRIAM's 8 microservices + a Java-based target application's 6
...
- **A frontend container in dev mode (`vite`/`webpack-dev-server` with hot
  ...
  Windows bind mount.** Encountered under real conditions (a target
  application's client mounted as a volume in its dev-server
  container, run via `vite --host 0.0.0.0`): a source file created/edited
...
12. [ ] Wire up authentication (Gateway + Frontend): decide the family
    ...
    an identity disconnected from the user's real account. **Append
    `?login_hint=<value>` to the forced-consent redirect URL** (§4bis) —
    already read and forwarded by `PRIAM-Frontend`, nothing to build there,
    just don't forget the query parameter on the target-application side.
