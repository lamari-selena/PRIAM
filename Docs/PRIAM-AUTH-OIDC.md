...
- A `keycloak` service added to the root `docker-compose.yml`
  ...
  not start at all. Encountered under real conditions.
- `Keycloak/priam-realm-realm.json` defines: the `priam-realm` realm, a public
  ...
  proprietary API.
- A test user (`priam-demo-subject` / `PriamTest123!`, attribute
  `idReference: "1"`, matching a real target application's data subject
  `id=1`).
- `.env`:
  ```bash
...
- A dedicated test user, `priam-demo-controller` / `OwnerPass123!`, distinct
  from `priam-demo-subject` (which stays the data-subject account on the
  `PRIAM-Frontend` side).
...
Example: a target application with no authorization server of its own. An
IdP (Keycloak or otherwise) **becomes**
...
accounts created — `priam-demo-subject` — are not linked to the target
application's own login system,
...
Example: a target application with its own system (a home-grown JWT, `POST
...
No case study integrated to date has a genuine natively
integrated IdP — each one so far either had no auth at all (Family 1) or a
...
application with a fixed domain (unlike a target application built for a
specific business domain — e-commerce, healthcare, personal finance, etc.) — the generated
...
6. **Test** (method from playbook §9 — proof by real state, not just the
   ...
   - A valid token (obtained through a real browser flow, or via a test using
     *Direct Grant* if enabled on the IdP client — document the exact curl
     commands used in your own integration's test log) → `200`, and the
...
- `PRIAM-Frontend-Provider` (§5bis): authentication, CORS, and JWT verified
  (the `priam-demo-controller` token accepted, no token → `401`), dashboard functional.
...
- The generic delegation SPI (Family 2) — would still need to be written and
  ...
  an existing target application's own accounts rather than standalone IdP
  accounts.