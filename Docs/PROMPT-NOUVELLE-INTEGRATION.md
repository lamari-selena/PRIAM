# Prompt to give an AI for a new PRIAM integration

> Copy-paste the block below into a fresh session (no prior context), replacing
> `<APP_NAME>` and `<APP_PATH>` with the real case study. Works equally well to
> resume an integration after a failure as to start a brand-new one.
...
microservices + 2 Angular frontends) into **<APP_NAME>**, whose code lives in
`<APP_PATH>`. PRIAM itself lives at the root of this repository/folder.
...
3. Open `<APP_NAME>`'s real code (ORM models, existing routes) — never invent
...
4. If you see `Docs/SESSION-HANDOFF-*.md` or any file in `Docs/` documenting a
   ...
   find it there).
5. Before touching the code: launch `<APP_NAME>` on its own (without PRIAM) and
...
1. `Databases/db_insertion_script.sql` — annotation of `<APP_NAME>`'s real
   ...
   (§8.1.b).
2. The 4 Provider bridge endpoints on the `<APP_NAME>` side (§2) —
...
5. **Bidirectional, persistent navigation between `<APP_NAME>` and PRIAM** —
   not just the forced sign-up redirect (point 4): a visible button/link in
   `<APP_NAME>` (e.g. in account settings) letting an already-registered user
   ...
   of `<APP_NAME>`, not just its default root URL.
...
8. Docker wiring (`common_network`, `PRIAM_CDP_URL`/`PRIAM_ACTOR_URL`,
   `CUSTOM_PROVIDER_URL` pointing directly at `<APP_NAME>`) — §5. **If another
...
9. If a Keycloak IdP is wired up (§6) **and** `<APP_NAME>` has its own local
...
- **PRIAM stays generic: 0 lines of code changed in the PRIAM
  ...
  generic PRIAM bug (not specific to `<APP_NAME>`) through a real test — only
...
- **Minimize the number of lines added on the `<APP_NAME>` side** — reuse the
...
1. `case-studies/<APP_NAME>/priam-integration/INTEGRATION-REPORT.md` — the
...
2. `case-studies/<APP_NAME>/priam-integration/ETAPES-FAITES.md` — the raw
   ...
   `/data`, `/provider` prefixes; the Provider bridge port on the `<APP_NAME>`
