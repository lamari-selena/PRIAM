...
│
├── PRIAM-Services/                  # All PRIAM microservices and frontends
...
3. **Authentication bridge** — bridge the application's existing accounts to PRIAM's OIDC-based Gateway (any OIDC-compliant provider, not just Keycloak — see `Docs/PRIAM-AUTH-OIDC.md`) so users aren't asked to authenticate a second time. Either the provider becomes the application's identity source directly, or it delegates to the application's existing login (SSO) — see `Docs/PRIAM-AUTH-OIDC.md` §7 for both patterns.
...
- A Docker network named `common_network` shared with the target application's stack — create it once with `docker network create common_network`

Authentication (`PRIAM-Gateway` resource server + both `PRIAM-Frontend` and
`PRIAM-Frontend-Provider` as OIDC clients) is provider-agnostic — see
`Docs/PRIAM-AUTH-OIDC.md`. The root `docker-compose.yml` includes a `keycloak`
service (with a ready-to-use `Keycloak/priam-realm.json` realm, two clients —
`Data-client` for data subjects, `Provider-client` for application owners) as a
concrete, working OIDC provider out of the box; swap `CUSTOM_OIDC_ISSUER_URI`
to point at any other OIDC-compliant issuer instead if preferred.
...
| `CUSTOM_KEYCLOAK_URL` | `http://localhost:8080` | Legacy Keycloak base URL (external) — unrelated to the generic OIDC resource server below |
| `CUSTOM_CDP_URL` | `http://consent:8089` | Consent Decision Point URL |
| `CUSTOM_OIDC_ISSUER_URI` | empty (fail-open) | Gateway resource server issuer — see `Docs/PRIAM-AUTH-OIDC.md` |
| `CUSTOM_OIDC_JWK_SET_URI` | empty | Only needed if the Gateway container can't reach the issuer directly (`Docs/PRIAM-AUTH-OIDC.md` §4) |
| `CUSTOM_FRONTEND_ORIGINS` | `http://localhost:4200,http://localhost:4000` | CORS-allowed origins for `PRIAM-Frontend`/`PRIAM-Frontend-Provider` (`Docs/PRIAM-AUTH-OIDC.md` §4bis) |
